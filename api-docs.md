...
| GET | `/api/audio/<filename>` | Serves an audio file. ESP node ambience may include `?offset_s=<seconds>`; the server streams from that position (fallback path since 2026-08-31 — beds normally ride the live broadcast below). |
| GET | `/api/audio/live/<key>.mp3` | Shared realtime bed broadcast (live_audio.py, 2026-08-31): ONE ffmpeg per bed, every node hears the same live edge; laggards skip their backlog to the edge. Keys are per-bed, minted when a bed is dispatched; a stale key (server restart) returns 404 and the next dispatch replaces it. |
...
| POST | `/api/sound_mode` | `{"mode": "attended"\|"unattended"}` or `{"toggle": true}` — flips which sound selections play. Live beds (maze ambience, room backgrounds, Cuddle floor bed) whose pools differ between modes restart with a fresh pick (listed as `restarted` in the response); one-shots and cues pick the new mode up on their next draw. The sim's Sound Mode button, audio console, and Cuddle orb operator page all use this |
| GET | `/api/orb/page/<page>` | Plain-text Cuddle orb operator pages. `page` is `mode`, `health`, `maintenance`, or `projector`. `health` lists only configured node-audio rooms currently down; `projector` includes RS232 power status, renderer status, manual override, and floor theme |
| POST | `/api/orb/action` | Cuddle orb operator actions. Body: `{"action":"toggle_mode"}`, `restart_server`, `restart_projection`, `projector_on`, or `projector_off`. Restart and projector actions are queued on the Pi host from inside Docker; projector force actions also create `.projector-manual` |
