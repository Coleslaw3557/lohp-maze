"""Fastcon BLE controller (Broadlink Fastcon / BRMesh floods).

Forked from dennispg/esphome-fastcon (MIT, see LICENSE) with one addition:
app-free pairing, ported from dsclee1/BRmesh-esp32-mqtt — the controller
broadcasts the wake probe, collects unpaired lights (they advertise the
factory key 5e367bc4) and assigns them OUR mesh key + sequential light ids,
so no phone/BRMesh app ever owns the mesh. Requires esp32_ble_tracker for
the listening half; scanning only needs to run while pairing is active.
Protocol credit: mooody.me / ArcadeMachinist / dsclee1 / dennispg.
"""
...
FastconController = fastcon_ns.class_(
    "FastconController", cg.Component, esp32_ble_tracker.ESPBTDeviceListener
)
