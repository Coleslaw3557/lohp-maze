...
class RemoteHostManager:
    ...
    def update_client_rooms(self, unit_name, ip, rooms, websocket):
        ...
        self.connected_clients[ip] = websocket  # Store the WebSocket object
        self.remote_hosts[ip] = {"name": unit_name, "rooms": rooms}
        self.save_config()
        logger.info(f"Updated associated rooms for client {unit_name} ({ip}): {rooms}")
        for room in rooms:
            logger.info(f"Associating room {room} with client {unit_name} ({ip})")
