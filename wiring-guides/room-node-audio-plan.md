...
`audio_files/cues/`); music streams as before. Anywhere this doc mentions
...
#   media         GET /api/audio/<file>              (music bed)
```
...
1. `node_audio_manager.py` + `node_audio_config.json`: aioesphomeapi downlink
   ...
   the embedded `play_cue` action), `start_background_music` →
...
one audio source of truth on the server outweighs the delta). Music start
over HTTP (0.5–3 s) only affects the 300 s rotation, which is unsynced
anyway. Re-check the cue delta during the marginal-RF soak on the bench
checklist; a back-to-back cue replaces the one still playing.
