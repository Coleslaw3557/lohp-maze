...
_ap.add_argument('--theme', default='lava')
_ap.add_argument('--clip', default=None)
_ap.add_argument('--ss', default='6', help='clip timestamp of the frame to condition on')
_ARGS = _ap.parse_args()
THEME = _ARGS.theme
CLIP = _ARGS.clip or {'lava': 'clip_lava_raw.mp4', 'water': 'clip_water_raw.mp4'}[THEME]
...
eng = THEMES[THEME](layout, grid_w=GW_ENGINE)

raw = subprocess.run(
    ['ffmpeg', '-v', 'error', '-ss', _ARGS.ss, '-i',
     os.path.join(HERE, CLIP), '-frames:v', '1',
...
Image.fromarray(out).save(os.path.join(HERE, f'{THEME}_conditioned_960.png'))
Image.fromarray(out).resize((640, 480), Image.LANCZOS).save(
    os.path.join(HERE, f'{THEME}_conditioned_640.png'))
